  ...
  return (
    <div className="flex h-screen w-full bg-[#f4f5f7] font-sans text-[#333333]">
      ...
      <aside className="w-[240px] bg-white border-r border-[#e5e5e5] flex flex-col">
        <div className="h-[60px] flex items-center px-[24px] border-b border-[#e5e5e5]">
        ...
        <div className="p-[16px]">
          ...
          <nav className="space-y-[4px]">
            <div className={getMenuClass('首页')} onClick={() => { setActiveMenu('首页'); window.__setCurrentPage && window.__setCurrentPage('index'); }}>
              <i className="fas fa-home w-[20px] text-center mr-[8px]"></i> 首页
            </div>
            <div className={getMenuClass('产品管理')} onClick={() => { setActiveMenu('产品管理'); window.__setCurrentPage && window.__setCurrentPage('index'); }}>
              <i className="fas fa-th-large w-[20px] text-center mr-[8px]"></i> 产品管理
            </div>
            <div className={getResourceMenuClass()} onClick={() => setIsResourceOpen(!isResourceOpen)}>
              <div className="flex items-center">
                <i className="fas fa-folder-open w-[20px] text-center mr-[8px]"></i> 资源管理
              </div>
              <i className={`fas fa-chevron-${isResourceOpen ? 'up' : 'down'} text-[12px] text-[#666]`}></i>
            </div>
            {/* 资源管理子菜单 */}
            {isResourceOpen && (
            ...
            <div className={getMenuClass('设备管理')} onClick={() => { setActiveMenu('设备管理'); window.__setCurrentPage('deviceManage'); }}>
              <i className="fas fa-microchip w-[20px] text-center mr-[8px]"></i> 设备管理
            </div>
            <div className={getMenuClass('开发者中心')} onClick={() => setActiveMenu('开发者中心')}>
              <div className="flex items-center">
                <i className="fas fa-code w-[20px] text-center mr-[8px]"></i> 开发者中心
              </div>
              <i className="fas fa-chevron-down text-[12px] text-[#666]"></i>
            </div>
          </nav>
        </div>
        <div className="mt-auto p-[16px] border-t border-[#e5e5e5]">
          <div className="flex items-center px-[12px] py-[10px] text-[14px] cursor-pointer hover:bg-[#f5f5f5] rounded-[4px]">
            <i className="far fa-user w-[20px] text-center mr-[8px]"></i> 个人信息
          </div>
        </div>
      </aside>
      {/* 主内容区 */}
